...
export const CATEGORIES = {
  email: { label: 'Email', placeholder: '[EMAIL]', layer: 'regex', priority: 1 },
  ssn: { label: 'SSN', placeholder: '[SSN]', layer: 'regex', priority: 2 },
  card: { label: 'Credit card', placeholder: '[CARD]', layer: 'regex', priority: 3 },
  ip: { label: 'IP address', placeholder: '[IP]', layer: 'regex', priority: 4 },
  phone: { label: 'Phone', placeholder: '[PHONE]', layer: 'regex', priority: 5 },
...
function aggregate(tokens) {
  ...
  for (const tok of tokens) {
    ...
    const [prefix, tag] = entity.includes('-') ? entity.split('-') : ['B', entity]
    ...
    if (contiguous) {
      ...
      flush()
      cur = { type, words: [isPiece ? word.slice(2) : word], score: tok.score ?? 1 }
    }
  }
  flush()
  return groups
...
export function mergeSpans(regexSpans, nerSpans) {
  ...
  for (const span of nerSpans) {
    const clash = regexSpans.find((r) => span.start < r.end && span.end > r.start)
    if (!clash) kept.push(span)
  }
